/* ***************************************************************
 *
 * PROCEDURE:	get_current.cc
 *
 * PROJECT:		dataman client side c++ routines
 * 
 * DATE:		Fri Apr 18 21:30:23 MDT 2003
 * 
 * AUTHOR:		Tom Green
 * 
 * FILES:
...
/*
 * gets the current key and it's associated record.
 */
/*
...
int index::get_current()
{
    ...
    if (this->_curkey.get_len() == 0)
        comm.db_err(ENOGET, "%s: Can't get_current", _progname);

	if (this->_idxno < 0 || this->_idxno > MAX_INDEX || this->_fno < 0 || this->_fno > this->_nfiles)
		comm.db_err(0, "%s: memory corruption detected in get_current", _progname);

	sprintf(msg, "%d|%d|%" PRId64 "|%" PRId64 "|%d|", GET_CURRENT, this->_idxno,
	...
	try {
		...
		comm.db_err(0, "%s: error reading server socket in get_current", _progname);
	}

	i = atoi(ret);
	if (i < 0)
		comm.db_err(i, "%s: error during get_current", _progname);
	else if (i == 0) {
		delete[] ret;
		return(FALSE);
	}
