/* ***************************************************************
 *
 * PROCEDURE:	get_prior.cc
 *
 * PROJECT:		dataman client side c++ routines
 * 
 * DATE:		Fri Apr 18 21:21:36 MDT 2003
 * 
 * AUTHOR:		Tom Green
 * 
 * FILES:
...
/*
 * get the previous key and its record from an index
 */
/*
...
int index::get_prior()
{
    ...
    if (this->_curkey.get_len() == 0)
        comm.db_err(ENOGET, "%s: Can't get_prior", _progname);
	...
	try {
		...
		comm.db_err(0, "%s: error reading server socket in get_prior", _progname);
	}

	i = atoi(ret);
	if (i < 0)
		comm.db_err(i, "%s: error during get_prior", _progname);
	else if (i == 0) {
		delete[] ret;
		return(FALSE);
	}
