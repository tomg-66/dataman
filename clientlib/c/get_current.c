...
DATAMAN_API int db_g_curr(char *index_name)
{
	...
	if (!idx->_curkey) {		// initial get never performed.
	...
	i = sprintf(msg, "%d|%d|%"PRIu64"|%"PRIu64"|%d|", GET_CURRENT, idx->_idxno,
	...
	if (i < 1) {
		if (i < 0)
			db_err(i, "%s: error during get_current", _progname);
		free(ret);
		return(FALSE);
	}
/*
...
invalid_response:
	db_err(EINVMSG, "%s: invalid GET_CURRENT response", _progname);
	free(ret);
	return FALSE;
}
