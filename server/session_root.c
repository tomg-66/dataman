 ...
 * This is metadata cleanup, not transaction abort or crash recovery.
 */
static void reap(void)
{
	struct shmid_ds status;
	for (int i = 0; i < MAX_CONNS; ++i) {
		if (roots[i].path && shmctl(roots[i].shmid, IPC_STAT, &status) < 0 &&
...
{
	char *end, *field, *path, *canonical, *last, *parent;
	long pid;
	int shmid, slot = -1, result = EINVMSG;
	struct stat status;
	struct shmid_ds ipc;
	if (!cmd || !data || offset < 1 || (size_t)offset >= strlen(cmd)) return EINVMSG;
	/* Internal requests carry the connection process PID before the command. */
	errno = 0;
	pid = strtol(cmd, &end, 10);
	if (errno || end == cmd || *end != '|' || pid < 1 || pid > INT_MAX) return EINVMSG;
	shmid = shmget((key_t)pid, 0, 0);
	if (shmid < 0 || shmctl(shmid, IPC_STAT, &ipc) < 0) return ENOSHM;
	field = cmd + offset;
	if (mode == 2) {
		/* MKIDX: key length | index name | database root | file count | ... */
		for (int i = 0; i < 2; ++i) {
			end = strchr(field, '|');
			if (!end || end == field) return EINVMSG;
			field = end + 1;
		}
	}
	end = strchr(field, '|');
	if (!end || end == field || (mode != 2 && end[1])) return EINVMSG;
	path = strndup(field, (size_t)(end - field));
	if (!path) return ENOALLOC;
	if (mode == 1) {
		/* Match the trailing /files/<workfile>, not an arbitrary /files/
		 * substring somewhere in a root's own name. */
		last = strrchr(path, '/');
		if (!last || !last[1]) goto bad_path;
		*last = 0;
		parent = strrchr(path, '/');
		if (!parent || strcmp(parent + 1, "files")) goto bad_path;
		if (parent == path) parent[1] = 0;
		else *parent = 0;
	}
	canonical = realpath(path, NULL);
	free(path);
	if (!canonical) return errno == ENOMEM ? ENOALLOC : ENOFILE;
	if (stat(canonical, &status) < 0 || !S_ISDIR(status.st_mode)) {
		free(canonical); return ENOFILE;
	}
	pthread_mutex_lock(&roots_mutex);
	reap();
	for (int i = 0; i < MAX_CONNS; ++i) {
		if (roots[i].path && roots[i].shmid == shmid) { slot = i; break; }
		if (!roots[i].path && slot < 0) slot = i;
	}
	if (slot < 0) { result = ENOWSP; goto done; }
	if (roots[slot].path && (strcmp(roots[slot].path, canonical) ||
		roots[slot].device != status.st_dev || roots[slot].inode != status.st_ino)) goto done;
	if (mode) {
	...
	if (!roots[slot].path) {
		roots[slot].path = canonical; canonical = NULL;
